...
public class DFSBFSDisconnected {
    private List<List<Integer>> adjList;
    private int vertices;

    DFSBFSDisconnected(int vertices){
...
// undirected 
    ...
    void DFSRecursive(){
        ...
        for(int start =0; start<adjList.size(); start++){
            if(!visited[start]){
                System.out.println("Component starts at "+ start +" : ");
                dfsHelper(start, visited);
                System.out.println();
            }
        }
    ...
    public static void main(String[] args) {
        DFSBFSDisconnected graph = new DFSBFSDisconnected(8);
        graph.addEdge(0, 1);
        graph.addEdge(1, 2);
        graph.addEdge(2, 4);
        graph.addEdge(3, 0);
